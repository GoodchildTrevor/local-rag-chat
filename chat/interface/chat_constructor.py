...
from config.consts.chat_messages import (
    ENTER, GREETINGS, MODEL_RESPOND, ENTER,
    DOC_STANDBY, MODEL_STANDBY, MAIN_MENU,
...
def create_chat_page(
    ...
    """Setup NiceGUI interface for the chatbot"""
    prefix = tab.prefix
    header = tab.header
    ...
    def chat_page() -> None:
        ...
        with ui.row().classes("w-[90%] h-screen"):
            with ui.column().classes("w-[15%] bg-gray-100 p-4 h-full border-r"):
                ui.button(MAIN_MENU, on_click=lambda: ui.navigate.to("/")) \
                    .classes("w-full mb-6") \
                    .props("color=blue outline")
                
                ui.label("Настройки").classes("text-sm font-medium mt-6 mb-2")

            with ui.column().classes("flex-1 h-screen w-[70%] mx-auto p-4"):
                    ...
                    with history_ui:
                        ...
                        if sender == "user":
                            ...
                            with ui.row().classes("items-start gap-3 mb-4 w-full"):
                                ...
                                with ui.column().classes("flex-1"):
                                    if isinstance(tab, RAGTabConfig):
                ...
                async def clear_history() -> None:
                    ...
                    with history_ui:
                        render_message(GREETINGS.format(name=header), enter=ENTER, system=True)
