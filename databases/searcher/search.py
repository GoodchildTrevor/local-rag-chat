...
def dense_search(
    ...
    top_dense_hits = [hit for hit in dense_hits if hit.score >= cosine_similarity_threshold]

    if not top_dense_hits:
        logger.warning("No relevant cached answers found above similarity threshold")
        return list()
    
    max_rating = 3
    best_hit = None

    for hit in top_dense_hits:
        ...
        if best_hit:
            ...
            logger.info(f"Found a relevant cached answer with score: {best_hit.score} and rating: {max_rating}")
            return [
                HybridHit(
                    id=best_hit.id,
